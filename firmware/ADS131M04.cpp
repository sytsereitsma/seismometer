...
void ADS131M04::begin(uint8_t cs_pin, uint8_t drdy_pin)
{
  // Set pins up
  ...
  digitalWrite(ADS131M04_CS_PIN, 1);
...
adcOutput ADS131M04::readADC(void)
{
  ...
  uint8_t statush = SPI.transfer(0x00);
  uint8_t statusl = SPI.transfer(0x00);
  (void)SPI.transfer(0x00);

  res.status = ((statush << 8) | statusl);

  auto readNextChannel = []() -> int32_t
  {
      uint8_t x = SPI.transfer(0x00);
      uint8_t x2 = SPI.transfer(0x00);
      uint8_t x3 = SPI.transfer(0x00);

      int32_t value = (((x << 16) | (x2 << 8) | x3) & 0x00FFFFFF);
      if (value > 0x7FFFFF)
      {
        value = ((~(value)&0x00FFFFFF) + 1) * -1;
      }

      return value;
  };

  res.ch0 = readNextChannel();
  res.ch1 = readNextChannel();
  res.ch2 = readNextChannel();
  res.ch3 = readNextChannel();
